...
class MainWindow(QMainWindow):
    ...
    def _build_shell(self):
        ...
        side.addSpacing(14)
        library = QLabel("LIBRARY", self.sidebar)
        ...
        for key, text in (("documents", "Documents"), ("archives", "Compressed"), ("music", "Music"), ("videos", "Videos")):
            ...
            side.addWidget(button)
        side.addStretch(1)
        self.nav_status = QLabel("Ready", self.sidebar)
        ...
        title = QLabel("Downloads", content)
        title.setObjectName("pageTitle")
        subtitle = QLabel("Fast, reliable and organized downloads", content)
        ...
        actions = QHBoxLayout()
        add = QPushButton("+  New Download", content)
        ...
        actions.addWidget(QLabel("Concurrent", content))
        self.concurrency = QSpinBox(content)
        ...
        tabs.addStretch(1)
        self.filter_hint = QLabel("All downloads", content)
        ...
        footer = QHBoxLayout()
        self.status = QLabel("Ready", content)
        self.status.setObjectName("statusLine")
        footer.addWidget(self.status)
        footer.addStretch(1)
        self.connection_status = QLabel("●  System ready", content)
    ...
    def _set_nav_status(self, text: str):
        try:
            self.nav_status.setText(text)
            return
        except RuntimeError:
            self.nav_status = QLabel(text, self.sidebar)
            self.nav_status.setObjectName("navStatus")
            self.sidebar.layout().addWidget(self.nav_status)
    ...
    def _apply_filters(self):
        ...
        for task_id, row in self.rows.items():
            ...
            visible += int(show)
        self._set_nav_status(f"Showing {visible} download{'s' if visible != 1 else ''}")
    ...
    def _insert_row(self, task_id, name, status, downloaded, total, speed, priority):
        ...
        for symbol, tip, callback in (("Ⅱ", "Pause", self._row_pause), ("▶", "Resume / Retry", self._row_resume), ("🗑", "Delete", self._row_delete)):
            button = QPushButton(symbol, action_widget)
